...
// GenerateCRS creates a Common_Reference_String with degree p
func GenerateCRS(p int) *Common_Reference_String {
	var group = edwards25519.NewBlakeSHA256Ed25519()
	var rand = group.RandomStream()
	CRS := new(Common_Reference_String)
	CRS.G = make([](kyber.Point), p+1)
	CRS.Group = edwards25519.NewBlakeSHA256Ed25519()
	// Generate dim random generators
	for i := 0; i <= p; i++ {
		// For simplicity, using big.Int, you may need to adjust based on your requirements
		CRS.G[i] = group.Point().Pick(rand)
	}
	CRS.H = group.Point().Pick(rand)
	return CRS
}
...
// Neg computes the component-wise minus of the polynomials p and q and returns it
// as a new polynomial.
func (p *PriPoly) Neg(q *PriPoly) (*PriPoly, error) {
	coeffs := make([]kyber.Scalar, p.Threshold())
	for i := range coeffs {
		if i < q.Threshold() {
			tmp := q.g.Scalar().Neg(q.coeffs[i])
			coeffs[i] = p.g.Scalar().Add(p.coeffs[i], tmp)
		} else {
			coeffs[i] = p.coeffs[i]
		}
	}
...
// BatchCommit Generate Batch Commit for a size B of secrets
func BatchCommit(CRS *Common_Reference_String, PolyR []*PriPoly, p int) []kyber.Point {
	var BCom = make([]kyber.Point, len(PolyR))
	for i := 0; i < len(PolyR); i++ {
		BCom[i] = CRS.Group.Point().Null()
	}
	var Coff_S = make([][]kyber.Scalar, len(PolyR))
	for k := 0; k < len(PolyR); k++ {
		Coff_S[k] = PolyR[k].coeffs
	}
	for j := 0; j < len(PolyR); j++ {
		for i := 0; i < p+1; i++ {
			BatchCom_temp := CRS.Group.Point().Mul(Coff_S[j][i], CRS.G[j])
			BCom[j] = CRS.Group.Point().Add(BatchCom_temp, BCom[j])
		}
	}
	return BCom
}

// Poly_Struct_Switch switches the row and line for a 2-dim PriPoly
func Poly_Struct_Switch(Poly [][]*PriPoly, B int, N int) [][]*PriPoly {
	Poly_Switch := make([][]*PriPoly, N)
	for j := 0; j < N; j++ {
		Poly_Switch[j] = make([]*PriPoly, B)
	}
	for j := 0; j < N; j++ {
		for k := 0; k < B; k++ {
			Poly_Switch[j][k] = Poly[k][j]
		}
	}
	return Poly_Switch
}

// GenerateEvaluationPoints generates {y1-yn} as the input of inner product proof
func GenerateEvaluationPoints(group kyber.Group, p int, n int) [][]kyber.Scalar {
	res := make([][]kyber.Scalar, p)
	for i := 0; i < p; i++ {
		for j := 0; j < n; j++ {
			temp := group.Scalar().SetInt64(int64(math.Pow(float64(j+1), float64(i)))) //(i+1)**j
...
// Get_Poly_value get the Scalar value of the PriPoly
func Get_Poly_value(Poly_S_T [][]*PriPoly) [][]kyber.Scalar {
	var Coff_S = make([][]kyber.Scalar, len(Poly_S_T))
	for k := 0; k < len(Poly_S_T); k++ {
		for i := 0; i < len(Poly_S_T[0]); i++ {
			fmt.Println(Poly_S_T[k][i].coeffs)
			Coff_S[k] = append(Coff_S[k], Poly_S_T[k][i].coeffs...)
		}
	}
	return Coff_S
}
