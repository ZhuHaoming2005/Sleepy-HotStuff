...
// Ambiguity warning: should be "isLive"?
// Otherwise, if the connection does not exist, why the func returns false?
// Update: if connection[key]=true, key is not alive.
// If key is found to be not alive, connection[key] will be set to true.
// So when the map is initialized, all connections are viewed as alive.
