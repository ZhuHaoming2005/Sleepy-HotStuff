...
func (s *IntByteMap) InsertAll(input map[int][]byte) {
	s.m = input
}
...
func (s *IntStringMap) Init() {
	s.Lock()
	defer s.Unlock()
	s.m = make(map[int]string)
}

func (s *IntStringMap) Get(key int) (string, bool) {
	s.RLock()
	defer s.RUnlock()
	_, exist := s.m[key]
	if exist {
...
func (s *IntStringMap) GetAll() map[int]string {
	return s.m
}

func (s *IntStringMap) Insert(key int, value string) {
	s.Lock()
	defer s.Unlock()
	s.m[key] = value
}

func (s *IntStringMap) Delete(key int) {
	s.Lock()
	defer s.Unlock()
	_, exist := s.m[key]
	if exist {
